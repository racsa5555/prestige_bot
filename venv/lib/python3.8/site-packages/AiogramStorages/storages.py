...
class PGStorage(BaseStorage):
    ...
    async def get_state(self, *, chat: Union[str, int, None] = None, user: Union[str, int, None] = None,
                        default: Optional[str] = None,key = None) -> Optional[str]:
        db = await self.get_db() 
    ...
    async def get_data(self, *, chat: Union[str, int, None] = None, user: Union[str, int, None] = None,
                       default: Optional[dict] = None,key = None) -> Dict:
        db = await self.get_db()
    ...
    async def update_data(self,data: Dict = None, **kwargs):
        if data is None:
            data = {}
        temp_data = await self.get_data(chat=data['chat'], user=data['user'], default={})
        temp_data.update(data, **kwargs) 
        await self.set_data(chat=data['chat'], user=data['user'], data=temp_data)
